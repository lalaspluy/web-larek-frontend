...
- ApiPostMethods
  Тип-объединение: 'POST' | 'PUT' | 'DELETE'
...
Типы данных:
- EventName: string | RegExp - имя события или регулярное выражение
...
Типы данных:
- CatalogChangeEvent:
  catalog: ProductItem[] - массив товаров

- TFormErrors: объект с ошибками формы

Интерфейсы:
- IAppState - состояние приложения
- IProduct - данные продукта
- IOrder - данные заказа
- IOrderFormFirst - поля первого шага заказа
- IOrderFormSecond - поля второго шага заказа
- IBasketItem - элемент корзины для отображения
...
- ISuccessActions
  onClick: () => void - обработчик клика на кнопку закрытия
...
- ApiListResponse<Type> - тип ответа API для списков (наследуется от базового Api)
